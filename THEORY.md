...
| `summary` | 事件触发时输出矩阵指纹、路线、预条件器代价、最终验收残差、内存摘要和环境信息 | 通常很低；若需真实残差，至多增加一次算子应用 | 历史建议 |
...
默认 `TelemetryPolicy` 为 `level=:off` 且 `emit_on=()`。它只返回解和求解状态，不写入 `HistoryStore`，不保存轨迹，也不为遥测重新计算残差。`summary`、`trace` 与 `diagnostic` 不应强制每一步重新计算残差。若后端已经维护递推残差，则直接记录该值；若用户要求真实残差，验收器才执行 $b-Ax$ 的额外计算。`trace` 默认只在预设间隔采样，不保存全部迭代向量。条件数估计、谱估计和高频内存采样属于 `diagnostic`，不进入默认路径。
...
推荐的历史策略是 `level=:summary`，并设置 `emit_on=(:new_fingerprint, :failure, :fallback, :periodic)`。这会为首次出现的标签组合、失败和回退保留完整摘要，而不是为每个内层线性系统写入一条记录。只有研究特定迭代行为时，才显式使用 `emit_on=(:every_solve,)` 或 `level=:trace`。
