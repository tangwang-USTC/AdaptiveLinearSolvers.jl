...
| `MINOR` | 新增兼容的路线、后端扩展、预条件器、证据类型或文档章节 |
| `PATCH` | 修复实现、文档、测试或数值判定错误，且不改变公共语义 |

在 `0.y.z` 阶段，`MINOR` 可以包含试验性 API 的不兼容调整；从 `1.0.0` 起严格遵循稳定 API 承诺。
...
建议在 GitHub 上保护 `main`：要求拉取请求、至少一次审查、通过测试、禁止强推。发布由受保护 `main` 创建带注释 tag `vMAJOR.MINOR.PATCH`，再生成 GitHub Release；未经明确授权不得自动推送、创建 Release 或修改远程设置。
...
4. 提交到 `main` 后创建 `vMAJOR.MINOR.PATCH` tag。
5. 创建 GitHub Release，并链接对应 tag 的文档快照。

## 6. 初始仓库状态

当前仅建立文档基线。本地 Git 仓库可先使用项目分支承载首次提交；GitHub 远程、许可证、包 UUID、`Project.toml` 与源程序目录在项目身份确认后再创建。
