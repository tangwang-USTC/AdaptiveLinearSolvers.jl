...
| `BATCH` | 当前阶段内的小批量修复或补充，例如实现、文档、测试或数值判定修复，且不改变公共语义 |
...
当前版本为 `0.1.3`：主版本 `0` 表示试验期，阶段 `1` 表示直接路线数学核心，小批量改动版 `3` 记录公共 `solve(A, b; kwargs...)` 入口前置的源代码整理。GitHub 远程、许可证与公开发布仍需在项目身份确认后单独执行。
