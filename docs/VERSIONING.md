...
当前版本为 `0.0.4`：主版本 `0` 表示试验期，阶段 `0` 表示初始基线尚未进入独立能力阶段，小批量改动版 `4` 归并条件诊断、可观测性与历史建议、资源预算和后端能力登记。`0.0.4` 已在 Julia `1.12.6`、Krylov `0.10.9`、Serialization `1.11.0` 下通过 `85/85` 包测试，耗时约 `19.1 s`。后续零散修改仅记录为 Git commit；满足批量发布阈值后再递增版本。GitHub 远程、许可证与公开发布仍需在项目身份确认后单独执行。
