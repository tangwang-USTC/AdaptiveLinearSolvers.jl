...
当前版本为 `0.0.1`：主版本 `0` 表示试验期，阶段 `0` 表示初始基线尚未进入独立能力阶段，小批量改动版 `1` 归并当前直接路线、数学资格门、残差验收、按需审计与本地测试配置。后续零散修改仅记录为 Git commit；满足批量发布阈值后再递增版本。GitHub 远程、许可证与公开发布仍需在项目身份确认后单独执行。
