...
当前版本为 `0.0.2`：主版本 `0` 表示试验期，阶段 `0` 表示初始基线尚未进入独立能力阶段，小批量改动版 `2` 归并纯规划层、迭代方法资格、预条件器固定性语义和迭代路线规划。`0.0.2` 已在 Julia `1.12.0` 下通过 `29/29` 包测试。后续零散修改仅记录为 Git commit；满足批量发布阈值后再递增版本。GitHub 远程、许可证与公开发布仍需在项目身份确认后单独执行。
